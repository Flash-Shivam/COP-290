...
vector<float> mult_cblas(vector<vector<float> > a, vector<vector<float> > b,int n, int m);
...
void mult(string matrix , string kernel, int matrix_size, int kernel_size,int padsize){
  ...
  ker = parseMatrix(kernel,kernel_size);
  ans = mult_cblas(padding(mat,matrix_size,padsize),ker,matrix_size+2*padsize,kernel_size);
...
if(m>=n){
  ...
  for(int i=0;i<t*t;i++)
	...
	f = (float *)malloc( (t*t)*(m*m)*sizeof( float ));
    g = (float *)malloc( (m*m)*(1)*sizeof( float ));
    h = (float *)malloc( (t*t)*(1)*sizeof( float ));
	...
	cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 
                t*t,1 , m*m, alpha, f,m*m, g, 1, beta, h, 1);
...
/*
vector<float> mult_oblas(vector<vector<float> > a,vector<vector<float> > b,int n, int m){
			int i,j,k,p,q;
  int t = n-m+1;
	//cout<<"t="<<t<<endl;
  vector<vector<float> > c;
...
if(m>=n){
  ...
  for(int i=0;i<t*t;i++)
	...
	float alpha = 1.0 , beta = 0.0 ;
	cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 
                t*t,1 , m*m, alpha, f, k, g, 1, beta, h, 1);
		
		vector<float> mat(t*t);
...
/*
float *multt(vector<vector<float> > a,vector<vector<float> > b,int n, int m){
			int i,j,k,p,q;
  int t = n-m+1;
	//cout<<"t="<<t<<endl;
  vector<vector<float> > c;
